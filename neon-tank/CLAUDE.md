...
folder. It covers **NEON TANK only** — the repo root holds two other unrelated
games, each with its own folder.

## Overview

This folder is a single-file browser game: **NEON TANK** (Phase 4), a top-down
...
- **Single file, no dependencies.** Keep additions inline in `neon-tank/index.html` unless asked
  to restructure.
...
- **Stay inside this folder.** `neon-orbit/` and `echo-dungeon/` are separate games
  that share nothing with this one — no common assets, no shared code.
